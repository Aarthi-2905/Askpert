...
import { setStatus, fetchUseremail } from 'utils/Auth'; // Adjust the path as needed
...
jest.mock('utils/Auth', () => ({
    setStatus: jest.fn(),
    fetchUseremail: jest.fn(() => 'test@example.com'), // Mock the function and return a value
    logout: jest.fn(),
}));
...
describe('DashSidebar Component', () => {
    ...
    test('opens profile modal when clicking on Profile', async () => {
        ...
        await waitFor(() => {
            expect(fetchUseremail).toHaveBeenCalled(); // Check that the mock function was called
            expect(screen.getByDisplayValue('test@example.com')).toBeInTheDocument(); // Validate the mocked return value
        });
    });
    ...
    test('handles sign out import.meta', async () => {
        render(
            <MemoryRouter>
                <DashSidebar />
            </MemoryRouter>
    ...
    test('fetches user data on component mount', async () => {
        renderWithRouter(<DashSidebar />);
        await waitFor(() => {
            expect(fetch).toHaveBeenCalledWith(`${import.meta.env.VITE_HOST}:${import.meta.env.VITE_PORT}/verify/`, expect.any(Object));
        });
    });
    ...
    test('updates profile information', async () => {
        ...
        await waitFor(() => {
            expect(fetch).toHaveBeenCalledWith(
                `${import.meta.env.VITE_HOST}:${import.meta.env.VITE_PORT}/verify/`, expect.any(Object));
        });
    });
});